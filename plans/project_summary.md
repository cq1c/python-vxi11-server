...
> **核心目标**：产出一份**可信的、有证据的、能跨工具读、能长期不腐烂**的项目骨架文档
> **设计原则**：流水线分阶段执行 · 每阶段有可验证产物 · 逻辑环必须带证据 · 文档只承载稳定信息 · 跨工具兼容（AGENTS.md 标准）
...
1. **没读过的代码不能写进文档。** 任何描述必须可追溯到 `文件:行号` 或 `类.方法`（仅在中间产物中），最终对外文档使用语义路径。
...
5. **文档只承载稳定信息。** 几个月内会变化的内容（具体行号、当前实现细节、文件位置）不要写进最终文档。问自己："三个月后这句话还成立吗？"
...
> 注意：loops_draft.md 是中间产物，可以保留具体行号。但阶段 4 写入 LOOPS.md 时**必须把行号去掉，改为语义路径**。
...
| 不变量 | ✅ "同一订单同时只允许一个 worker 处理" | ❌ 锁的具体 key 命名规则 |
...
3. **`setup_agent_links.sh`** — 跨工具兼容软链接脚本
...
- 重构内部实现但保持接口
- 修复 bug、优化性能
- 重命名变量/方法（除非是 LOOPS.md 中提到的语义角色名）
- 调整代码组织（除非改变了模块职责）
...
> ⚠️ **关键约束**：LOOPS.md 中**不要写任何 `文件:行号` 引用**。改为"语义路径"——告诉 agent "在哪个模块、哪个语义角色"，agent 用 grep 一次就能找到。这样代码重构后文档不会失效。
...
### 跨工具兼容脚本：`setup_agent_links.sh`

```bash
#!/bin/bash
# 为不自动识别 AGENTS.md 的工具创建软链接
# AGENTS.md 是单一真相源，其他文件都是软链接

set -e

# Claude Code（截至 2026 年仍使用 CLAUDE.md）
[ ! -e CLAUDE.md ] && ln -sf AGENTS.md CLAUDE.md && echo "✓ CLAUDE.md → AGENTS.md"

# Cursor 旧版（新版已支持 AGENTS.md，按需启用）
# [ ! -e .cursorrules ] && ln -sf AGENTS.md .cursorrules && echo "✓ .cursorrules → AGENTS.md"

# Gemini CLI（格式可能不完全兼容，按需启用）
# [ ! -e GEMINI.md ] && ln -sf AGENTS.md GEMINI.md && echo "✓ GEMINI.md → AGENTS.md"

echo ""
echo "Agent context links set up. AGENTS.md is the single source of truth."
echo "When you update AGENTS.md, all linked files update automatically."
```
...
- [ ] 维护规则部分明确了"默认不更新"
- [ ] 提供了 setup_agent_links.sh 跨工具兼容脚本
- [ ] 已知盲点（⚠️ 项）显式列出

最终回答这个问题：**如果三个月后这份文档没人维护，它会变成"误导 agent 的过期信息"还是"依然有用的稳定描述"？**
...
每隔一段时间（建议大版本前），让 agent 执行以下检查：

1. 检查 LOOPS.md 中提到的"语义路径"是否还能 grep 到对应代码
2. 检查 AGENTS.md 中的命令是否还能跑通
3. 检查"不可违反的约束"是否还有人遵守（grep 反模式特征）
4. 检查"已知盲点"中的⚠️项是否已解决，已解决的移到正文，未解决的保留
...
- `AGENTS.md`（根目录）
- `LOOPS.md`（根目录）
- `setup_agent_links.sh`（根目录，可执行）
- `README.md` 追加章节
- 子目录 `AGENTS.md`（如有特殊约定的目录）
- `.docs-init/` 工作目录（保留作为证据库）
